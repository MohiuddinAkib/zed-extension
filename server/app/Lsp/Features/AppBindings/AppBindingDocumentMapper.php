...
class AppBindingDocumentMapper extends DocumentMapper
{
    ...
    {
        $indexed = $this->project->index->appBindings()
            ->map(fn (array $binding, string $key): array => ['key' => $key, ...$binding]);

        $coreBindings = collect(AppBindingContainerTypeMap::all())->map(fn (string $class, string $key) => [
            'key' => $key,
            'class' => $class,
            'path' => null,
            'line' => null,
        ...
        return $coreBindings->merge($indexed)->values();
    }
}
