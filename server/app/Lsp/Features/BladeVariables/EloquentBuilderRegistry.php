...
{
    /**
     * Fallback overlay for common Eloquent and Database Query Builder methods.
     *
     * Reflected project/framework metadata is the source of truth when available.
     * This list exists only for non-Laravel fixtures or failed runtime indexing.
     *
    ...
    public const BUILDER_METHODS = [
        ...
        'whereBinary' => [
            'signature'      => '(string $column, mixed $operator = null, mixed $value = null, string $boolean = \'and\'): \Illuminate\Database\Eloquent\Builder<static>',
            'return'         => '\Illuminate\Database\Eloquent\Builder<Model>',
            'snippet'        => 'whereBinary(${1})',
            'requiredParams' => 1,
            'doc'            => 'Add a binary comparison where clause to the query.',
        ],
        'whereAll' => [
            'signature'      => '(array $columns, string $operator, mixed $value = null, string $boolean = \'and\'): \Illuminate\Database\Eloquent\Builder<static>',
            'return'         => '\Illuminate\Database\Eloquent\Builder<Model>',
            'snippet'        => 'whereAll(${1})',
            'requiredParams' => 2,
            'doc'            => 'Add a "where all" clause comparing multiple columns with an operator.',
        ],
        'whereAny' => [
            'signature'      => '(array $columns, string $operator, mixed $value = null, string $boolean = \'and\'): \Illuminate\Database\Eloquent\Builder<static>',
            'return'         => '\Illuminate\Database\Eloquent\Builder<Model>',
            'snippet'        => 'whereAny(${1})',
            'requiredParams' => 2,
            'doc'            => 'Add a "where any" clause matching any of the specified columns.',
        ],
        'whereNone' => [
            'signature'      => '(array $columns, string $operator, mixed $value = null, string $boolean = \'and\'): \Illuminate\Database\Eloquent\Builder<static>',
            'return'         => '\Illuminate\Database\Eloquent\Builder<Model>',
            'snippet'        => 'whereNone(${1})',
            'requiredParams' => 2,
            'doc'            => 'Add a "where none" clause matching none of the specified columns.',
        ],
        'toRawSql' => [
            'signature'      => '(): string',
            'return'         => 'string',
            'snippet'        => 'toRawSql()',
            'requiredParams' => 0,
            'doc'            => 'Get the raw SQL representation of the query with embedded bindings.',
        ],
        'dumpRawSql' => [
            'signature'      => '(): static',
            'return'         => '\Illuminate\Database\Eloquent\Builder<Model>',
            'snippet'        => 'dumpRawSql()',
            'requiredParams' => 0,
            'doc'            => 'Dump the raw SQL query with embedded bindings.',
        ],
        'vector' => [
            'signature'      => '(string $column, array $vector, ?string $distance = null): \Illuminate\Database\Eloquent\Builder<static>',
            'return'         => '\Illuminate\Database\Eloquent\Builder<Model>',
            'snippet'        => 'vector(${1})',
            'requiredParams' => 2,
            'doc'            => 'Add pgvector similarity search condition.',
        ],
        'cosineDistance' => [
            'signature'      => '(string $column, array $vector): \Illuminate\Database\Eloquent\Builder<static>',
            'return'         => '\Illuminate\Database\Eloquent\Builder<Model>',
            'snippet'        => 'cosineDistance(${1})',
            'requiredParams' => 2,
            'doc'            => 'Order by pgvector cosine distance.',
        ],
        'l2Distance' => [
            'signature'      => '(string $column, array $vector): \Illuminate\Database\Eloquent\Builder<static>',
            'return'         => '\Illuminate\Database\Eloquent\Builder<Model>',
            'snippet'        => 'l2Distance(${1})',
            'requiredParams' => 2,
            'doc'            => 'Order by pgvector Euclidean (L2) distance.',
        ],
        'find' => [
     ...
     */
    public static function getMembersForModel(?string $modelClass = null, bool $isStatic = false, array $indexedMethods = []): array
    {
        $members = self::getIndexedMembersForModel($indexedMethods, $modelClass, $isStatic);

        foreach (self::getFallbackMembersForModel($modelClass, $isStatic) as $name => $member) {
    ...
    /**
     * Get fallback builder members formatted for LSP completion.
     *
     * @return array<string, array<string, mixed>>
     */
    protected static function getFallbackMembersForModel(?string $modelClass = null, bool $isStatic = false): array
    {
    ...
    /**
     * Format indexed reflected methods as LSP completion members.
     *
     * @param  array<int, array<string, mixed>>  $indexedMethods
     * @return array<string, array<string, mixed>>
     */
    protected static function getIndexedMembersForModel(array $indexedMethods, ?string $modelClass = null, bool $isStatic = false): array
    {
        $modelDisplay = $modelClass !== null ? class_basename($modelClass) : 'Model';
        $fullModel = $modelClass !== null ? '\\' . ltrim($modelClass, '\\') : 'Model';
        $members = [];

        foreach ($indexedMethods as $method) {
            if (!is_array($method)) {
                continue;
            }
            ...
            [$paramSignature, $requiredParams] = self::formatIndexedParameters(
                is_array($method['parameters'] ?? null) ? $method['parameters'] : [],
            );
            $returnType = self::normalizeIndexedReturnType((string) ($method['return'] ?? 'mixed'), $fullModel);
            ...
            $members[$name] = [
                ...
                'isMethod'       => true,
                'documentation'  => '**' . ($isStatic ? "{$modelDisplay}::{$name}" : "\${$modelDisplay}Query->{$name}") . "**\n\n```php\npublic function {$name}{$detail};\n```\n\nReflected from the installed Laravel framework.\n\n*Origin:* `Indexed Eloquent Builder`",
            ];
        }
     ...
     */
    protected static function formatIndexedParameters(array $parameters): array
    {
        $parts = [];
        $requiredParams = 0;

        foreach ($parameters as $parameter) {
            $normalized = self::normalizeIndexedParameter($parameter);

            if ($normalized === null) {
                continue;
            }

            if (!$normalized['hasDefault'] && !$normalized['isVariadic']) {
                $requiredParams++;
            }

            $part = $normalized['type'] !== '' && $normalized['type'] !== 'mixed' ? "{$normalized['type']} " : '';
            $part .= $normalized['isPassedByReference'] ? '&' : '';
            $part .= $normalized['isVariadic'] ? '...' : '';
            $part .= '$' . $normalized['name'];

            if ($normalized['hasDefault']) {
                $part .= ' = ' . $normalized['default'];
            }
    ...
    /**
     * @return array{name: string, type: string, hasDefault: bool, default: string, isVariadic: bool, isPassedByReference: bool}|null
     */
    protected static function normalizeIndexedParameter(mixed $parameter): ?array
    {
        if (is_string($parameter)) {
            return self::normalizeParameterString($parameter);
        }
        ...
        return [
            ...
            'default'             => (string) ($parameter['default'] ?? $parameter['defaultValue'] ?? 'null'),
            'isVariadic'          => (bool) ($parameter['isVariadic'] ?? false),
            'isPassedByReference' => (bool) ($parameter['isPassedByReference'] ?? false),
        ];
    }
     ...
     * @return array{name: string, type: string, hasDefault: bool, default: string, isVariadic: bool, isPassedByReference: bool}|null
     */
    protected static function normalizeParameterString(string $parameter): ?array
    {
        ...
        if (!preg_match('/^(?:(.+?)\s+)?(&)?(\.\.\.)?\$?([a-zA-Z_][a-zA-Z0-9_]*)(?:\s*=\s*(.+))?$/', $parameter, $matches)) {
            return null;
        }

        return [
            'name'                => $matches[4],
            'type'                => trim((string) ($matches[1] ?? 'mixed')) ?: 'mixed',
            'hasDefault'          => array_key_exists(5, $matches) && trim((string) $matches[5]) !== '',
            'default'             => trim((string) ($matches[5] ?? 'null')) ?: 'null',
            'isVariadic'          => ($matches[3] ?? '') === '...',
            'isPassedByReference' => ($matches[2] ?? '') === '&',
        ];
    }

    protected static function normalizeIndexedReturnType(string $returnType, string $fullModel): string
    {
        ...
        if (in_array($returnType, ['static', 'self', '$this'], true)) {
            return "\\Illuminate\\Database\\Eloquent\\Builder<{$fullModel}>";
        }

        return str_replace(
            ['TModelClass', 'TModel', 'Model', 'static', 'self', '$this'],
            [$fullModel, $fullModel, $fullModel, $fullModel, $fullModel, $fullModel],
            $returnType,
        );
    }
}
